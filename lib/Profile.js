...
var hiddenProperties=["_exe","_glob","_dir","_recursive","_extensions","_tags","_gridDir","_defaultGrid","_plugins","_ignore","_extra"];
var Profile=function(properties){
  ...
  Object.defineProperty(this,"defaultGrid",{
    enumerable: true,
    get: this.getDefaultGrid,
    set: this.setDefaultGrid
  });
...
Profile.prototype.addExe=function(exe){
  if(!exe){
    return;
  }
  if(Array.isArray(exe)){
    ...
    var e=path.resolve(this._cwd,String(exe));
    this._exe[e]=true;
  }
};
...
Profile.prototype.getExe=function(){
  var exe=[];
  for(var e in this._exe){
    if(this._exe.hasOwnProperty(e) && this._exe[e]){
      exe.push(e);
    }
  }
  return exe;
...
Profile.prototype.setGridDir=function(gridDir){
  gridDir=gridDir?String(gridDir):path.join(__dirname,"..","grid");
  this._gridDir=path.resolve(this._cwd,gridDir);
};

Profile.prototype.getDefaultGrid=function(){
  return this._defaultGrid;
};

Profile.prototype.setDefaultGrid=function(defaultGrid){
  this._defaultGrid=defaultGrid?path.resolve(this._cwd,String(defaultGrid)):null;
};
