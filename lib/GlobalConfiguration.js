...
GlobalConfiguration.logAppenders=["console","nostegama"];
GlobalConfiguration.logDir=path.join(os.tmpdir(),TMP_SUB_DIR,"logs");
GlobalConfiguration.overwrite=false;
GlobalConfiguration.saveGridImages=true;
...
try{
  ...
  console.warn(i18n.__("Failed to open config.json. Nostegama will continue to operate with default settings. Reason: %s",String(e)));
  // "Failed to open config.json. Application will continue to operate with default and cli settings."
}
if(configJSON){
  if(configJSON.general){
    if((typeof configJSON.general.saveGridImages)==="boolean"){
      GlobalConfiguration.saveGridImages=configJSON.general.saveGridImages;
    }
  }
...
var options=cli.parse({
  "userdata-dir": [false,i18n.__("The userdata directory of Steam. This option is only necessary if Nostegama is not able to locate the directory by itself."),"directory",null],
  "input": ["i",i18n.__("The JSON-file Nostegama shall process"),"file",null],
  ...
  "overwrite": ["o",i18n.__("Overwrites the shortcuts.vdf instead of complementing it. All previous Non-Steam Games will get lost."),"boolean",null]
});
...
if(options["overwrite"]){
  GlobalConfiguration.overwrite=options["overwrite"];
}

Object.defineProperty(GlobalConfiguration,"backup",{
  get: function(){
    return GlobalConfiguration.backupShortcuts && GlobalConfiguration.backupGrids;
  }
});
